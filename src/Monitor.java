...
public class Monitor implements MonitorInterface {
    ...
    public boolean fireTransition(int transition) {
        ...
        boolean k = true;
        while (k) {
            // Check if the transition is enabled by tokens first
            if (petriNet.isSensitizedByTokens(transition)) {
                
                // If it is a timed transition, check if the minimum time has elapsed
                if (petriNet.isTransitionTimed(transition)) {
                    long now = System.currentTimeMillis();
                    long sensTime = petriNet.getSensitizationTime(transition);
                    long elapsed = now - sensTime;
                    long tMin = petriNet.getTransitionTMin(transition);

                    if (elapsed < tMin) {
                        long sleepTime = tMin - elapsed;
                        
                        // Release the lock before sleeping so other threads can work
                        mutex.release();
                        
                        try {
                            Thread.sleep(sleepTime);
                        } catch (InterruptedException e) {
                            return false; // Interrupted during sleep, exit
                        }

                        // Re-acquire the mutex to enter the monitor again
                        try {
                            mutex.acquire();
                        } catch (InterruptedException e) {
                            return false;
                        ...
                        // Force the loop to continue and re-verify sensitization state
                        k = true;
                        continue;
                    }
                }

                // If we get here, it is sensitized by tokens AND time (or not timed)
                // We fire it!
            ...
            // If the transition is not enabled by tokens, it goes to sleep.
            } else {
